...
- [Night Check](https://github.com/CjDabrow/night-check) - Reviews Midnight code (Compact contracts, proof-server config, SDK) for the privacy and security traps specific to Midnight, each with a severity and fix, then certifies the review on-chain with a privacy-preserving Compact contract that writes only a commitment to the report to the ledger
